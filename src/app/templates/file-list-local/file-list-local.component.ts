...
import {FileLocalPermission, OvicFile, SimpleFileLocal} from "@model/file";
import {FileService} from "@service/core/file.service";
import {MediaService} from "@service/core/media.service";
import {AuthService} from "@service/core/auth.service";
import {NotificationService} from "@appNotification";
import {WAITING_POPUP} from "@utilities/syscats";
  ...
  onSelectFiles(event: Event) {
    if (event.target['files'].length) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        const tail = file['name'].slice(file['name'].lastIndexOf('.')).toLowerCase();
